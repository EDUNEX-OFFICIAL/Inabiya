...
**19 August 2026:** the client has **started testing and using** those three.  
...
## 7. Next product after shop / ops / CMS

Engineering already has **MVP code** for the Journal and Creator Collective. Those were **out of** the 18 August handover certificate. They are not client-tested like the shop.

| Order | Product | Why this order |
|---|---|---|
| **1 (next)** | **Editorial / blog** (`/articles`, editorial CMS) | PRD Stage 2. Content/trust for the same public brand. One-sided (writers, SEO, medical). |
| **2 after that** | **Creator campaigns** (`/creator`) | PRD Stage 3. Two-sided marketplace (brand + creator + escrow). Harder while payments are still mock. |
| Paused | Procurement (suppliers / POs) | Not in this pack. Client stock buy stays inventory qty + optional amount. |

While the client tests the shop: **fix UAT bugs first**, then start Editorial. Do not run Journal and Creator as two active product streams at once.

**Editorial first slice:** public articles + specialist pages + assign → gates → publish. Not related-articles / public comments / RSS unless that slice finishes early.
