...
export default function ProductDetailPage({ params }: { params: { slug: string } }) {
  ...
  async function addToBox() {
    ...
    try {
      ...
      router.push('/build-your-box?continue=1');
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Failed');
      setBusyBox(false);
    }
  ...
  if (error && !product) {
    return (
      <main className="gift-page">
        <p>{error}</p>
        <Link href="/products" className="gift-link mt-gs-3 inline-block text-body">
          ← All products
        </Link>
      </main>
    );
  ...
  return (
    <main className="gift-page flex flex-col gap-gs-6 sm:gap-gs-7">
      ...
      >
        <Link href="/" className="gift-link shrink-0">
          Gift
        </Link>
        ...
        {primaryCollection ? (
          ...
          <>
            <Link href="/products" className="gift-link shrink-0">
              Products
            </Link>
            <span aria-hidden className="shrink-0">
              /
          ...
          {message ? (
            <p className="mt-gs-3 text-body text-success" role="status">
              ...
              {message === 'Added to cart' || /^Added \d+ to cart$/.test(message) ? (
                <>
                  {' '}
                  <Link href="/cart" className="gift-link font-medium">
                    View cart →
                  </Link>
                </>
              ) : null}
...
function ProductDetailsBand({ product }: { product: CatalogProduct }) {
  ...
  const recipientCollection: Record<string, string> = {
    girl: '/collections/for-baby-girl',
    boy: '/collections/for-baby-boy',
    mom: '/collections/for-expecting-mom',
    unisex: '/collections/unisex-gifts',
  };
  const ageCollection: Record<string, string> = {
    newborn: '/collections/newborn',
    infant: '/collections/infant',
    toddler: '/collections/toddler',
  };
  const occasionCollection: Record<string, string> = {
    'welcome-baby': '/collections/welcome-baby',
    'baby-shower': '/collections/baby-shower',
    naming: '/collections/naming-ceremony',
    birthday: '/collections/first-birthday',
  };

  const tags: Array<{ href: string; label: string }> = [];
  for (const c of product.collections) {
    tags.push({ href: `/collections/${c.slug}`, label: c.title });
  }
  for (const t of product.recipientTags ?? []) {
    tags.push({
      href: recipientCollection[t] ?? `/products?recipient=${t}`,
      label: labelTag(t),
    });
  }
  for (const t of product.ageBands ?? []) {
    if (t === 'any') continue;
    tags.push({
      href: ageCollection[t] ?? `/products?age=${t}`,
      label: labelTag(t),
    });
  }
  for (const t of product.occasionTags ?? []) {
    tags.push({
      href: occasionCollection[t] ?? `/products?occasion=${t}`,
      label: labelTag(t),
    });
  }
  ...
  return (
    <section>
      <div className="flex flex-wrap items-end justify-between gap-gs-3">
        ...
        {collectionSlug ? (
          <Link href={`/collections/${collectionSlug}`} className="gift-link text-body">
            Browse collection
          </Link>
        ) : (
          <Link href="/products" className="gift-link text-body">
            Browse all
          </Link>
        )}
      </div>
