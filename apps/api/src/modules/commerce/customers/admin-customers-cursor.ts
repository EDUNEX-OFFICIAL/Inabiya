/**
 * Keyset cursor for admin customers desk (createdAt DESC, id DESC).
 * Opaque: base64url(`${isoCreatedAt}_${uuid}`)
 */

const UUID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;

export type AdminCustomerCursor = {
  createdAt: Date;
  id: string;
};

export function encodeAdminCustomerCursor(row: { createdAt: Date; id: string }): string {
  return Buffer.from(`${row.createdAt.toISOString()}_${row.id}`, 'utf8').toString('base64url');
}

export function decodeAdminCustomerCursor(raw: string): AdminCustomerCursor {
  let decoded: string;
  try {
...
export function adminCustomerKeysetAfter(cursor: AdminCustomerCursor): {
