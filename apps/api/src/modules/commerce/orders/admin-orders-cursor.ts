/**
 * Keyset cursor for admin orders queue (createdAt DESC, id DESC).
...
export function encodeAdminOrderCursor(row: { createdAt: Date; id: string }): string {
  return Buffer.from(`${row.createdAt.toISOString()}_${row.id}`, 'utf8').toString('base64url');
}

export function decodeAdminOrderCursor(raw: string): AdminOrderCursor {
  let decoded: string;
  try {
...
export function adminOrderKeysetAfter(cursor: AdminOrderCursor): {
